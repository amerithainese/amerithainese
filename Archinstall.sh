...
# Easy-Arch (unencrypted BTRFS + snapshots) installer
...
kernel_selector () {
    ...
    info_print "1) Stable: Vanilla Linux kernel (linux)"
    info_print "2) Hardened: Security-focused kernel (linux-hardened)"
    info_print "3) Longterm: LTS kernel (linux-lts)"
    info_print "4) Zen Kernel: Desktop-optimized kernel (linux-zen)"
    ...
    case $kernel_choice in
        ...
        * ) error_print "You did not enter a valid selection, please try again."; return 1;;
    esac
}
...
network_selector () {
    ...
    info_print "3) wpa_supplicant + dhcpcd: WiFi utility + DHCP client"
...
network_installer () {
    case $network_choice in
        ...
        4 ) info_print "Installing dhcpcd."
            ...
            systemctl enable dhcpcd --root=/mnt &>/dev/null
            ;;
        5 ) info_print "Skipping network installation (manual setup).";;
    esac
}
...
locale_selector () {
    input_print "Please insert the locale you use (format: xx_XX.UTF-8. Enter empty to use en_US.UTF-8, or \"/\" to search locales): "
    read -r locale
    case "$locale" in
        '') locale="en_US.UTF-8"
...
======================================================================
${RESET}"
info_print "Welcome to easy-arch (unencrypted BTRFS + snapshots) installer."
...
# Creating a new partition scheme (ESP + ROOT).
...
# Formatting the ESP as FAT32 and the ROOT as BTRFS.
...
# Mounting the ROOT to create subvolumes.
mount "$ROOT" /mnt
...
mount -o "$mountopts",subvol=@home "$ROOT" /mnt/home
mount -o "$mountopts",subvol=@root "$ROOT" /mnt/root
mount -o "$mountopts",subvol=@srv "$ROOT" /mnt/srv
mount -o "$mountopts",subvol=@snapshots "$ROOT" /mnt/.snapshots
mount -o "$mountopts",subvol=@var_log "$ROOT" /mnt/var/log
...
sed -i "/^#\?$locale/s/^#//" /mnt/etc/locale.gen || true
...
# Configuring /etc/mkinitcpio.conf (no sd-encrypt here).
info_print "Configuring /etc/mkinitcpio.conf."
cat > /mnt/etc/mkinitcpio.conf <<EOF
HOOKS=(systemd autodetect keyboard sd-vconsole modconf block filesystems)
EOF

# Configure and finish setup inside the new system.
info_print "Configuring the installed system (timezone, initramfs, Snapper, GRUB)."
arch-chroot /mnt /bin/bash -e <<'EOF_CHROOT'

# Set timezone (attempt to auto-detect from IP; fallback to UTC if tool unavailable)
if command -v curl >/dev/null 2>&1; then
    TZ=$(curl -s http://ip-api.com/line?fields=timezone || true)
else
    TZ=""
fi
if [[ -n "$TZ" ]]; then
...
# remove any leftover mount points first
if mountpoint -q /.snapshots; then
    umount /.snapshots || true
fi
rm -rf /.snapshots || true
snapper --no-dbus -c root create-config /
# remove the temporary subvolume created by snapper create-config, it will be managed properly
...
# Setting user password and creating user if requested.
if [[ -n "$username" ]]; then
    echo "%wheel ALL=(ALL:ALL) ALL" > /mnt/etc/sudoers.d/wheel
    info_print "Adding the user $username to the system with wheel privileges."
...
sed -Ei 's/^#(Color)$/\1\nILoveCandy/;s/^#(ParallelDownloads).*/\1 = 10/' /mnt/etc/pacman.conf || true
...
for service in "${services[@]}"; do
    systemctl enable "$service" --root=/mnt &>/dev/null || true
done

# Final message
info_print "Done — the system has been installed to the disk you selected."
info_print "You may now reboot. To make further changes, chroot into /mnt."
exit 0
